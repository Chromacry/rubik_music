...
public class GeneratePlaylistDataActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_generate_playlistdata);
        ...
        //Init database
        databaseReference = FirebaseDatabase.getInstance().getReference("playlistCollection"); //For Adding Songs
...
//        String songFileLink = editSongFileLink.getText().toString().trim();
        ...
        ArrayList mySongsList = new ArrayList<>();
        mySongsList.add("-N92eaYoFQ_kLmkvQ8Ka");
        mySongsList.add("-N91ac7H2tP5WlqVJPis");
        mySongsList.add("-N8uS85bhjoZ3BrPivR2");


        //checking if the value is provided
        if (!TextUtils.isEmpty(songTitle) && !TextUtils.isEmpty(songArtist) && !TextUtils.isEmpty(songGenre) && !TextUtils.isEmpty(songCategory) && !TextUtils.isEmpty(songImageLink)) {
            //getting a unique id using push().getKey() method
            //create a unique id and we will use it as the Primary Key for our songData
            String id = databaseReference.push().getKey();

            //creating an Artist Object
            GeneratePlaylistData songData = new GeneratePlaylistData(id, songTitle, songArtist, songGenre, songCategory, songImageLink, mySongsList);
